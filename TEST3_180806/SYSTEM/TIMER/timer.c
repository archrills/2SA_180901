...
extern  char buf[128];	
extern  unsigned try6;
extern unsigned char run_direction;
extern unsigned char encoder_ready;
extern unsigned char timer5_count;
unsigned char timer7_count=0;
extern  unsigned char fault_register;
extern  void relay_another_process(void);
...
void TIM3_IRQHandler(void)   //TIM3中断
{
	...
	else
	{
	   ...
	   relay_another_process();
	   LED_TORQUE=~LED_TORQUE;
	}	
 ...
 void Timer5_off(void)
{
	...
	TIM_TimeBaseStructure.TIM_Period = 350; //设置在下一个更新事件装入活动的自动重装载寄存器周期的值	 计数到5000为500ms
	TIM_TimeBaseStructure.TIM_Prescaler =(7200-1); //设置用来作为TIMx时钟频率除数的预分频值  10Khz的计数频率  
	TIM_TimeBaseStructure.TIM_ClockDivision = 0; //设置时钟分割:TDTS = Tck_tim
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;  //TIM向上计数模式
...
void Timer5_Init(void)
{
    ...
    TIM_TimeBaseStructure.TIM_Period = 395; //设置在下一个更新事件装入活动的自动重装载寄存器周期的值	 计数到5000为500ms
	TIM_TimeBaseStructure.TIM_Prescaler =(7200-1); //设置用来作为TIMx时钟频率除数的预分频值  10Khz的计数频率  
	TIM_TimeBaseStructure.TIM_ClockDivision = 0; //设置时钟分割:TDTS = Tck_tim
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;  //TIM向上计数模式
	TIM_TimeBaseInit(TIM5, &TIM_TimeBaseStructure); //根据TIM_TimeBaseInitStruct中指定的参数初始化TIMx的时间基数单位
...
void Timer4_Init(void)
{
	...
	TIM_TimeBaseStructure.TIM_Period = 250; //设置在下一个更新事件装入活动的自动重装载寄存器周期的值	 计数到5000为500ms
	TIM_TimeBaseStructure.TIM_Prescaler =(7200-1); //设置用来作为TIMx时钟频率除数的预分频值  10Khz的计数频率  
	TIM_TimeBaseStructure.TIM_ClockDivision = 0; //设置时钟分割:TDTS = Tck_tim
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;  //TIM向上计数模式
...
void pwm_excute(void)
{
   unsigned int i;
   unsigned long int middle_varible1,middle_varible2;

    if(percent_now<0)
    {									   
      middle_varible1=0;
    }
    else if(percent_now<1000)
    {
      middle_varible1=percent_now;
    }
    else
   {
      middle_varible1=1000;
   }

  middle_varible2=all_pwm_value_register;
  i=zero_pwm_value_register+(middle_varible1*middle_varible2)/1000;
  PWM(i);
